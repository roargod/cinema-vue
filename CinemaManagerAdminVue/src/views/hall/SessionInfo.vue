  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-row>
        <el-col :span="24">
          ...
          <el-button @click="getSessionList" style="font-size: 18px;">
            搜索
          </el-button>
          <el-button type="primary" @click="showAddDialog" style="font-size: 18px;">
            添加场次
          </el-button>
          <el-button type="danger" style="font-size: 18px;" @click="isAbleMultipleDelete">
            批量删除
          </el-button>
        </el-col>
      </el-row>
    ...
    <el-dialog title="添加场次" :visible.sync="addDialogVisible" width="60%" @close="addDialogClosed">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addSession" style="font-size: 18px;"> 确 定</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog title="修改场次" :visible.sync="editDialogVisible" width="60%" @close="editDialogClosed">
      <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="150px">
        ...
        <el-form-item label="场次日期" prop="sessionDate">
          <el-date-picker v-model="editForm.sessionDate" value-format="yyyy-MM-dd" type="date" placeholder="选择日期">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="播放时间" prop="sessionTime">
          <el-time-picker v-model="editForm.playTime" value-format='HH:mm' placeholder="选择播放时间">
          </el-time-picker>
        </el-form-item>
        <el-form-item label="场次价格" prop="sessionPrice">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="editSessionInfo" style="font-size: 18px;"> 确 定</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog title="查看座位" :visible.sync="checkDialogVisible" :width="checkDialogWidth">
      ...
      <br />
      <div class="row" v-for="(value, key) in seats" :key="value.id">
        <span style="margin-right: 100px">{{ key }}</span>
        <div style="display: flex; justify-content: flex-end">
          <span class="seat" :class="isSelected[seats[key][index]]" :key="item.id" v-for="(item, index) in value"></span>
        </div>
      </div>
    </el-dialog>
